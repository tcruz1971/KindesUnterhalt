...
public class WechselModelFormula {
    ...
    public static void main(String[] args) throws IOException {
        ...
        prt("Gesamtgehalt",
                fmtEU(totalGehalt));
        prt("Gehalt Vater  % vom Gesamtgehalt",
                Math.round((gehaltvater / totalGehalt) * 100) + "%");
        prt("Gehalt Mutter % vom Gesamtgehalt",
                Math.round((gehaltmutter / totalGehalt) * 100) + "%");

        lineBreak();

        prt("Total Kindergeld",
                fmtEU(total_kindergeld));
        
        lineBreak();
        
        for (int i = 0; i < kindermehrbedarf.length; i++) {
            prt(String.format("Kind %s Mehrbedarf", i + 1),
                    fmtEU(Double.valueOf(kindermehrbedarf[i])));
        }
        
        lineBreak();

        prt("Total Kindermehrbedarf",
                fmtEU(totalKinderZimmer));

        for (int i = 0; i < kindesunterhalt.length; i++) {
            prt(String.format("Kind %s Unterhalt nach Duesseldorfer Tabelle", i + 1),
                    fmtEU(Double.valueOf(kindesunterhalt[i])));
        }

        prt("Total Kindesunterhalt (Inkl.Kindermehrbedarf)",
        ...
        lineBreak();
        prt(String.format("Gehalt Vater  Kinderunterhalt (%s/%s)*%s",
                fmtEU(gehaltvaterNetto),
                fmtEU(totalGehaltNetto),
                fmtEU(totalKinderKost)),
                fmtEU(gehaltvaterKinderUnterhalt));

        prt(String.format("Gehalt Mutter Kinderunterhalt (%s/%s)*%s",
                fmtEU(gehaltmutterNetto),
                fmtEU(totalGehaltNetto),
                fmtEU(totalKinderKost)),
        ...
        prt(String.format("Gehalt Vater  Kinderunterhalt 50%% Wechselmodel (%s/2)",
                fmtEU(gehaltvaterKinderUnterhalt)),
                fmtEU(gehaltvaterKinderunterhalt50));
        prt(String.format("Gehalt Mutter Kinderunterhalt 50%% Wechselmodel (%s/2)",
                fmtEU(gehaltmutterKinderUnterhalt)),
        ...
        if (gehaltvaterKinderUnterhalt - (totalKinderKost / 2) > 0 && gehaltvaterKinderUnterhalt > gehaltmutterKinderUnterhalt) {
            prt(String.format("Vater zahlt Differenz (%s-%s)-(%s/2)",
                    fmtEU(gehaltvaterKinderunterhalt50),
                    fmtEU(gehaltmutterKinderunterhalt50),
                    fmtEU(total_kindergeld)),
                    fmtEU(gehaltvaterResult));
        } else {
            prt(String.format("Mutter zahlt Differenz (%s-%s)-(%s/2)",
                    fmtEU(gehaltmutterKinderunterhalt50),
                    fmtEU(gehaltvaterKinderunterhalt50),
                    fmtEU(total_kindergeld)),
